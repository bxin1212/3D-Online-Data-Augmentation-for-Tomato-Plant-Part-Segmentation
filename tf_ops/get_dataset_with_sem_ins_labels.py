...
def get_dataset(file_path, \
    ...
    for filename in f.keys():
        ...
        exclude_id = np.where((pc_raw[:, -1] == 2) | (pc_raw[:, -2] == 2) | (pc_raw[:, -1] == 5))
        ...
        # Get colour channels
        # pc_r_median = np.median(pc_excluded[:, np.arange(3, pc_excluded.shape[1]-2, 3)], axis=1) / 255
        # pc_g_median = np.median(pc_excluded[:, np.arange(4, pc_excluded.shape[1]-2, 3)], axis=1) / 255
        # pc_b_median = np.median(pc_excluded[:, np.arange(5, pc_excluded.shape[1]-2, 3)], axis=1) / 255
        ...
        pc_normal_color_label = np.c_[pc_normalised[:, 0:3], \
                                      ...
                                      pc_normalised[:, 3:48], \
                                      pc_excluded[:, -2], \
                                      pc_excluded[:, -1]]  # [x, y, z, norm_x, norm_y, norm_z, colors (3*15), instance labels, semantic labels]
        # pc_normal_color_label = np.c_[pc_normalised[:, 0:3], \
        #                               normal_array, \
        #                               #pc_normalised[:, 48], \
        #                               pc_normalised[:, 3:6], \
...
if __name__ == '__main__':
    ...
    isTestset = True
    isDown_sampling = False
    NUM_POINTS = 50000
    
    file_path = "../Original Labeled Point Cloud Dataset/without inner points/surface_pcs_with_semantic_instance_labels (refined) (40 pcs).h5"
