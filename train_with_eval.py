...
## - Dataset parameters
parser.add_argument('--dataset_path', default='./dataset/inner_point_removal/Pos_Norm_Color_Sem Ins_Test DS/training_set.h5', help='Dataset path')
parser.add_argument('--testset_path', default='./dataset/inner_point_removal/Pos_Norm_Color_Sem Ins_Test DS/test_set.h5', help='Dataset path')
...
parser.add_argument('--num_repeats', type=int, default=5, help='Number of training times for a single network configuration and dataset [default: 5]')
...
parser.add_argument('--aug_strategy', default=['random down sampling'], help='Log dir [default: none]')

parser.add_argument('--jitter_sigma', type=float, default=0.08, help='Sigma to generate the random disturbance for jittering [default: 0.01]')
parser.add_argument('--jitter_clip', type=float, default=0.40, help='Clip for the random disturbance of jittering [default: 0.05]')
...
parser.add_argument('--leaf_shift_sigma', type=float, default=0.70, help='Shift sigma for leaf translation augmentation method [default: 0.05]')
parser.add_argument('--leaf_shift_clip', type=float, default=3.5, help='Shift clip for leaf translation augmentation method [default: 0.25]')

parser.add_argument('--leaf_rotate_z_range', type=float, default=3*np.pi/4, help='Range for leaf rotation augmentation method around z axis [default: 3*np.pi/4]')
...
test_data = h5py.File(TESTSET_PATH, "r")
...
def train_with_eval():
    ...
    best_class_ave_f1_rec = np.zeros((1, NUM_REPEATS), dtype=np.float32)
    best_stemwork_f1_rec = np.zeros((1, NUM_REPEATS), dtype=np.float32)
    ...
    for repeat_id in range(NUM_REPEATS):
        ...
        val_file_list = np.array(list(test_data.keys()))
        ...
        np.savetxt(save_folder + "/eval_loss_rec.csv", eval_loss_rec)
        np.savetxt(save_folder + "/eval_overall_acc_rec.csv", eval_overall_acc_rec)

        for class_id in range(NUM_CLASSES):
            np.savetxt(save_folder + "/eval_rec_class_" + str(class_id) + ".csv", \
        ...
        best_class_ave_f1_rec[0, repeat_id] = best_class_ave_f1_this
        best_stemwork_f1_rec[0, repeat_id] = best_stemwork_f1_this
...
def get_batch(file_list, idxs, start_idx, end_idx, is_training=True, now_eval=False):
...
def train_one_epoch(sess, ops, train_writer, train_file_list, additional_input):
    ...
    for batch_idx in range(int(num_batches)):
        ...
        batch_data, batch_label = get_batch(train_file_list, train_idxs, start_idx, end_idx, is_training=is_training, now_eval=False)
    ...
    train_individual_class = np.zeros((3, NUM_CLASSES), dtype="float32")
    for i in range(NUM_CLASSES):
        ...
        train_individual_class[0, i] = recall
        train_individual_class[1, i] = precision
        train_individual_class[2, i] = f1
        log_string('train class ' + str(i) + ' F1 score: %f' % (f1))
    log_string('train avg class F1 score: %f' % (np.mean(train_individual_class[2,:])))
    ...
    return train_loss, \
           train_acc, \
           train_individual_class
        

def eval_one_epoch(sess, ops, validation_writer, val_file_list, repeat_nr):
    """ ops: dict mapping from string to tf ops """
    is_training = False
    ...
    log_string('---- Repeat No.%d ---- EPOCH %03d EVALUATION ----'%(repeat_nr, EPOCH_CNT))
    ...
    for batch_idx in range(int(num_batches)):
        ...
        cur_batch_data, cur_batch_label = get_batch(val_file_list, test_idxs, start_idx, end_idx, is_training=is_training, now_eval=True)
